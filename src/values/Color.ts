import { IntRange } from '../types/common-types'

export function useHexColorShade(hex: Hex, shade: IntRange<1, 100>) {
    const { hue, saturation } = useHexToHsl(hex);
    return useHslToHex({ hue, saturation, lightness: 100 - shade });
}

export function useHexToFilter(hex: Hex): CssFilter {
    const color = Color.fromHex(hex);
    return CssFilterReSolver.from(color).solve();
}

export function useHexToHsl(hex: Hex): HSL {
    return Color.fromHex(hex).toHSL();
}

export function useHslToFilter(hsl: HSL): CssFilter {
    const color = Color.fromHSL(hsl);
    return CssFilterReSolver.from(color).solve();
}

export function useHslToHex(hsl: HSL): Hex {
    return Color.fromHSL(hsl).toHex();
}

class CssFilterReSolver {
    private rgb: RGB;
    private hsl: HSL;
